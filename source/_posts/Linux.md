...
1. **`mkdir 目录名称`：** 直接mkdir会在当前文件夹中创建一个文件夹，但是要保证上级文件夹存在，加上参数 -p ，可以连同上级文件夹一起创建：

```bash
mkdir -p /home/yue/a/b/c
```
...
注意：rm不仅可以删除目录，也可以删除其他文件或压缩包，但是默认只能删除文件，为了增强大家的记忆，  无论删除任何目录或文件，都直接使用`rm -rf` 目录/文件/压缩包

```bash
rm -rf /home/yue/a/b   #文件夹b和子文件夹c一块儿删除了
```
...
4.**`mv 目录名称 新目录名称`：** 修改目录的名称 --**重命名**（改）

```bash
mv /home/test /home/test2  #将test文件夹重命名为test2
```
...
如将当前目录下的aaa文件复制到/tmp下，并命名为bbb：

```bash
cp ./aaa /tmp/bbb
```

复制目录要用-r，如复制/etc/这个目录下所有内容到/tem下：
...
**查看某个时间点的进程信息。**信息相对于top较少，且不是动态的
...
[参考资料]([https://github.com/Snailclimb/JavaGuide/blob/master/docs/operating-system/%E5%90%8E%E7%AB%AF%E7%A8%8B%E5%BA%8F%E5%91%98%E5%BF%85%E5%A4%87%E7%9A%84Linux%E5%9F%BA%E7%A1%80%E7%9F%A5%E8%AF%86.md](https://github.com/Snailclimb/JavaGuide/blob/master/docs/operating-system/后端程序员必备的Linux基础知识.md))
